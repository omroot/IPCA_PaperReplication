...
class ipca(object):
    def __init__(self, RZ: Optional[pd.DataFrame] = None, return_column: Union[int, str, tuple] = 0, 
                 X: Optional[pd.DataFrame] = None, W: Optional[pd.DataFrame] = None, 
                 Nts: Optional[pd.Series] = None, add_constant: bool = True) -> None:
        '''
        [Inputs]
    ...
    def fit(self, K: int = 1, OOS: bool = False, gFac: Optional[pd.DataFrame] = None, 
            normalization_choice: str = 'PCA_positivemean',
            normalization_choice_specs: Optional[List[str]] = None, OOS_window: str = 'recursive',
            OOS_window_specs: int = 60, factor_mean: str = 'constant', R_fit: bool = True,
            Beta_fit: bool = False, dispIters: bool = False, minTol: float = 1e-4, 
            maxIters: int = 5000, F_names: Optional[List[Union[str, int]]] = None, 
            G_names: Optional[List[str]] = None, R2_bench: str = 'zero', 
            dispItersInt: int = 100) -> Dict[str, Any]:
        '''
        [Inputs]
        ...
        return {'xfits' : fittedX,
                'Gamma' : Gamma,
                'Factor' : Factor,
                'Factors' : Factor,  # alias for compatibility
                'Lambda' : Lambda_dict,
                'rfits' : fittedR,
    ...
    def _linear_als_estimation(self, Gamma0: np.ndarray, K: int, M: int, KM: int, 
                              normalization_choice: str, normalization_choice_specs: Optional[List[int]],
                              gFac: Optional[pd.DataFrame], Dates: Optional[pd.Index] = None) -> Tuple[np.ndarray, np.ndarray]:
        '''
    ...
    def _svd_initial(self, K: int, M: int, gFac: Optional[pd.DataFrame] = None, 
                    Dates: Optional[pd.Index] = None) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        '''
    ...
    def _normalization_choice(self, Gamma: np.ndarray, Factor: np.ndarray, K: int, M: int, KM: int, 
                             normalization_choice: str, 
                             normalization_choice_specs: Optional[List[int]]) -> Tuple[np.ndarray, np.ndarray]:
        if M == KM:
    ...
    def _VARB(self, X: np.ndarray) -> np.ndarray:
    ...
    def _R2_calc(self, reals: pd.DataFrame, fits_total: pd.DataFrame, fits_pred: pd.DataFrame, 
                benchR2: Optional[pd.DataFrame] = None) -> Tuple[float, float]:
        if benchR2 is None:
    ...
    def R2_of_fits(self, results: Optional[Dict[str, Any]] = None, date_range: Optional[pd.Index] = None, 
                   benchR2: Optional[str] = None, R2name: Optional[str] = None, 
                   inplace: bool = True) -> Optional[Dict[str, Any]]:
        '''
    ...
    def _setup_fits(self, R_fit: bool, Beta_return: bool) -> Tuple[bool, bool, Dict[str, pd.DataFrame], Optional[Dict[str, pd.DataFrame]], Optional[pd.DataFrame]]:
        Rdo = False
    ...
    # find sublist
    def _find_sublist(self, sub: List[str]) -> List[int]:
        pos = list()
        ct=0
        for subj in sub:
            try:
